...
/*
LOGIN should stay visible for every non-admin state.
Only a verified SYNTHENOVA admin gets the ADMIN/LOGOUT UI.
*/
if (isAdmin === true) {
if (loginButton) {
...
if (authStatus) {
authStatus.textContent = currentUser ? "AUTHENTICATED" : "GUEST";
}
if (mobileAdmin) {
...
const targetPanel =
$(
`admin${capitalizeFirstLetter(tabName)}Panel`
);
if (targetPanel) {
...
try {
/*
Attach all UI controls BEFORE waiting for Supabase.
This guarantees the LOGIN button and menu remain clickable
even if Supabase configuration/network initialization fails.
*/
setupNavigation();
